...
ViBeBase::ViBeBase(
  ...
  const boost::uint32_t COLUMNS = width * channels;

  /* Creates the historyImage structure. */
  historyImage = new boost::uint8_t[NUMBER_OF_HISTORY_IMAGES * COLUMNS * height];

  for (boost::uint32_t i = 0; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
	  for (boost::int32_t index = COLUMNS * height - 1; index >= 0; --index)
      historyImage[i * COLUMNS * height + index] = buffer[index];
  }

  /* Now creates and fills the history buffer. */
  historyBuffer =
	  new boost::uint8_t[COLUMNS * height * (numberOfSamples - NUMBER_OF_HISTORY_IMAGES)];

  for (boost::int32_t index = COLUMNS * height - 1; index >= 0; --index) {
	  boost::uint8_t value = buffer[index];

	  for (boost::uint32_t x = 0; x < numberOfSamples - NUMBER_OF_HISTORY_IMAGES; ++x) {
      historyBuffer[index * (numberOfSamples - NUMBER_OF_HISTORY_IMAGES) + x] =
        min(
          max(
		  static_cast<boost::int32_t>(value)+rand() % 20 - 10, // Add noise.
		  static_cast<boost::int32_t>(BACKGROUND)
          ),
		  static_cast<boost::int32_t>(FOREGROUND)
        );
    }
  }

  /* Fills the buffers with random values. */
  boost::int32_t size = (width > height) ? 2 * width + 1 : 2 * height + 1;

  jump = new boost::uint32_t[size];
  neighbor = new boost::int32_t[size];
  position = new boost::uint32_t[size];

  for (boost::int32_t i = 0; i < size; ++i) {
...
boost::uint32_t ViBeBase::getNumberOfSamples() const {
  return numberOfSamples;
}

/******************************************************************************/

boost::uint32_t ViBeBase::getMatchingThreshold() const {
  return matchingThreshold;
}

/******************************************************************************/

void ViBeBase::setMatchingThreshold(boost::int32_t matchingThreshold) {
...
boost::uint32_t ViBeBase::getMatchingNumber() const {
  return matchingNumber;
}

/******************************************************************************/

void ViBeBase::setMatchingNumber(boost::int32_t matchingNumber) {
  if (matchingNumber <= 0)
    throw; // TODO Exception;
...
boost::uint32_t ViBeBase::getUpdateFactor() const {
  return updateFactor;
}

/******************************************************************************/

void ViBeBase::setUpdateFactor(boost::int32_t updateFactor) {
  if (updateFactor <= 0)
    throw; // TODO Exception;
  ...
  boost::int32_t size = 2 * max(width, height) + 1;
